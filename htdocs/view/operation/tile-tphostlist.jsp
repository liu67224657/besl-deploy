	...
	<table width="100%" border="0" cellspacing="0" cellpadding="1">
	  <tr class="toolbar">      
        ...
        <td height="24" width="240" align="center">
      		<s:if test="%{operation.status.preRun}"><a href="?env=${env}&oper=${oper}&args=${args}&deploydir=${deploydir}&exec=execute"><s:text name="def.exec.execute.name"/></a></s:if>
	    </td>
        <td height="24">
			<s:if test="operation.status.exit || operation.status.preRun"><a href="javascript:if(confirm('Do you want to remove the operation log?')){this.location='?env=${env}&oper=${oper}&args=${args}&app=${app}&deploydir=${deploydir}&exec=remove'}"><s:text name="def.exec.remove.name"/></a></s:if>
	    </td>
      </tr>
    </table>
    <table width="100%" border="1" cellspacing="0" cellpadding="1">
